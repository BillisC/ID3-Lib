...
void ID3v2::readULF(const unsigned int &cPos, const uint32_t &frSize){
    ...
    if(frameContent[frSize - 1] == '\0') std::cout << ch2str(frameContent, frSize - 1);
    else std::cout << ch2str(frameContent, frSize);
}
...
void ID3v2::readCOM(const unsigned int &cPos, const uint32_t &frSize){
    ...
    if(encoding == 0x01){
        ...
        std::cout << ch2str(frameContent, contentSize);
    }
}
