...
char *ID3v2::newFramePIC(const std::string &newFrameID, const std::string &imagePath, uint32_t &newFullFrameSize, const short int &tagVersion){
    ...
    uint32_t fileSize = std::filesystem::file_size(imagePath);
    ...
    int mimeSize = 0;
    std::string mimeType = "";
    if(std::filesystem::path(imagePath).extension() == ".jpg"){
        mimeSize = 10;
        mimeType = "image/jpeg";
    }
    else if(std::filesystem::path(imagePath).extension() == ".png"){
        mimeSize = 9;
        mimeType = "image/png";
    }
    else if(std::filesystem::path(imagePath).extension() == ".bmp"){
        mimeSize = 12;
...
bool ID3v2::set(const std::string &whatID, const std::string &content){
    ...
    short int tagVersion = 0;  // Tag version
